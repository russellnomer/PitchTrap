...
#      Pushes a clean SNAPSHOT (no old git history), so big files buried in
#      past commits can never break the push.
...
# --- 5. build a clean SNAPSHOT commit (no history) ----------------------------
# Uses a temporary index so the Repl's own git state is untouched, and the
# pushed commit has NO parents - old commits containing >100MB files (which
# GitHub rejects) are never sent.
SNAP_INDEX="$(mktemp -u)"
GIT_INDEX_FILE="$SNAP_INDEX" git add -A
TREE=$(GIT_INDEX_FILE="$SNAP_INDEX" git write-tree)
COMMIT=$(git commit-tree "$TREE" -m "Replit backup $(date +%F)")
rm -f "$SNAP_INDEX"
...
git push -f _backup "$COMMIT:refs/heads/$BRANCH"
